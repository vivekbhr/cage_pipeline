...
rule demultiplex_fastq:
    ...
    log:
        "02_splitting/Log/demultiplex.log"
    threads: 20
    shell:
        "export R_LIBS_USER="+config["R_libs_path"]+" && "+os.path.join(config["R_path"],"Rscript")+ " {params.rscript} {params.expname} "
        "{input.r1} {input.r2} {input.sampleinfo} {params.outdir} &> {log}"
...
if config["mapping_prg"] == "subread":
    rule subread:
        ...
        log:
            "03_mapping{}/Log/subread.log".format(spike_prefix)
        threads: 15
        shell:
...
elif config["mapping_prg"].upper().find("STAR") >=0:
    if config["paired"]:
        rule STAR:
            input:
                r1 = "02_splitting/{base}_R1.fastq.gz",
...
if not config["map_spike"]:
    rule bam_coverage:
        input:
            "03_mapping/{base}.bam.bai"
        output:
            "05_bigwigs/with_duplicates/{base}.bw"
        params:
            inputdir = "03_mapping/{base}.bam"
        log:
        ...
        shell:
            ...
            "bamCoverage "
            "--Offset 15 "
            "--normalizeUsing CPM -p 20 -bs 1 -b {params.inputdir} "
            "-o {output}  &> {log}"

    rule bam_coverage_withoutdup:
        input:
            "04_removedup/{base}.filtered.bam"
        output:
        ...
        shell:
            ...
            "bamCoverage "
            "--Offset 15 "
            "--normalizeUsing CPM -p 20 -bs 1 -b {input} "
            "-o {output}  &> {log}"
